// generated with ast extension for cup
// version 0.8
// 27/0/2022 19:47:41


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(Unmatched Unmatched) { }
    public void visit(ReturnType ReturnType) { }
    public void visit(Mulop Mulop) { }
    public void visit(GroupDeclList GroupDeclList) { }
    public void visit(Matched Matched) { }
    public void visit(Relop Relop) { }
    public void visit(MethodVarList MethodVarList) { }
    public void visit(ConditionList ConditionList) { }
    public void visit(OptionalSquareDesignator OptionalSquareDesignator) { }
    public void visit(CondTermList CondTermList) { }
    public void visit(MulopList MulopList) { }
    public void visit(VarDeclListModif VarDeclListModif) { }
    public void visit(Variable Variable) { }
    public void visit(StatementList StatementList) { }
    public void visit(ConstDeclVal ConstDeclVal) { }
    public void visit(Addop Addop) { }
    public void visit(Factor Factor) { }
    public void visit(FormParsList FormParsList) { }
    public void visit(Statements Statements) { }
    public void visit(ConstDeclList ConstDeclList) { }
    public void visit(NewFactorExprOptional NewFactorExprOptional) { }
    public void visit(OptionalFormalParams OptionalFormalParams) { }
    public void visit(ActParsList ActParsList) { }
    public void visit(OptionalPrintParam OptionalPrintParam) { }
    public void visit(Label Label) { }
    public void visit(VarDeclList VarDeclList) { }
    public void visit(OptionalActPars OptionalActPars) { }
    public void visit(AddopList AddopList) { }
    public void visit(ActPars ActPars) { }
    public void visit(MethodTypeName MethodTypeName) { }
    public void visit(DesignatorList DesignatorList) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(Statement Statement) { }
    public void visit(CondFact CondFact) { }
    public void visit(DesignatorFunctionCall DesignatorFunctionCall) { }
    public void visit(DesignatorElement DesignatorElement) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(OptionalMinus OptionalMinus) { }
    public void visit(GroupDecl GroupDecl) { }
    public void visit(FormPars FormPars) { }
    public void visit(NoConditionListDecl NoConditionListDecl) { visit(); }
    public void visit(ConditionListDecl ConditionListDecl) { visit(); }
    public void visit(Condition Condition) { visit(); }
    public void visit(NoCondTermListDecl NoCondTermListDecl) { visit(); }
    public void visit(CondTermListDecl CondTermListDecl) { visit(); }
    public void visit(CondTerm CondTerm) { visit(); }
    public void visit(ConditionFactWithRelopExpr ConditionFactWithRelopExpr) { visit(); }
    public void visit(ConditionFact ConditionFact) { visit(); }
    public void visit(FunctionCall FunctionCall) { visit(); }
    public void visit(Assignment Assignment) { visit(); }
    public void visit(Dec Dec) { visit(); }
    public void visit(Inc Inc) { visit(); }
    public void visit(OptionalPrintParamDerived2 OptionalPrintParamDerived2) { visit(); }
    public void visit(OptionalPrintParamDerived1 OptionalPrintParamDerived1) { visit(); }
    public void visit(OptionalPrintParamm OptionalPrintParamm) { visit(); }
    public void visit(DoStatementStart DoStatementStart) { visit(); }
    public void visit(AssignmentStatement AssignmentStatement) { visit(); }
    public void visit(MatchedDerived1 MatchedDerived1) { visit(); }
    public void visit(DoStatement DoStatement) { visit(); }
    public void visit(ContinueStatament ContinueStatament) { visit(); }
    public void visit(BreakStatment BreakStatment) { visit(); }
    public void visit(StatementsDecl StatementsDecl) { visit(); }
    public void visit(MatchedStatement MatchedStatement) { visit(); }
    public void visit(ReturnNoExpr ReturnNoExpr) { visit(); }
    public void visit(ReturnExpr ReturnExpr) { visit(); }
    public void visit(ReadStatement ReadStatement) { visit(); }
    public void visit(PrintStmt PrintStmt) { visit(); }
    public void visit(DesignatorStatement1 DesignatorStatement1) { visit(); }
    public void visit(UnmatchedIfElse UnmatchedIfElse) { visit(); }
    public void visit(UnmatchedIf UnmatchedIf) { visit(); }
    public void visit(UnmatchedStmt UnmatchedStmt) { visit(); }
    public void visit(MatchedStmt MatchedStmt) { visit(); }
    public void visit(NoStatementListDeclarations NoStatementListDeclarations) { visit(); }
    public void visit(StatementListDeclarations StatementListDeclarations) { visit(); }
    public void visit(ListStatement ListStatement) { visit(); }
    public void visit(NoActParsListAdd NoActParsListAdd) { visit(); }
    public void visit(ActParsDerived1 ActParsDerived1) { visit(); }
    public void visit(ActParsListAdd ActParsListAdd) { visit(); }
    public void visit(NoOptionalActParsDecl NoOptionalActParsDecl) { visit(); }
    public void visit(OptionalActParsDecl OptionalActParsDecl) { visit(); }
    public void visit(DesignatorWithActPars DesignatorWithActPars) { visit(); }
    public void visit(DesignatorElementSquar DesignatorElementSquar) { visit(); }
    public void visit(DesignatorElementWithDot DesignatorElementWithDot) { visit(); }
    public void visit(NoDesignatorListDeclarationMultiple NoDesignatorListDeclarationMultiple) { visit(); }
    public void visit(DesignatorListDeclarationMultiple DesignatorListDeclarationMultiple) { visit(); }
    public void visit(DesignatorName DesignatorName) { visit(); }
    public void visit(Designator Designator) { visit(); }
    public void visit(NoNewFactorExprOpt NoNewFactorExprOpt) { visit(); }
    public void visit(NewFactorExprOpt NewFactorExprOpt) { visit(); }
    public void visit(ExprFactor ExprFactor) { visit(); }
    public void visit(NewFactor NewFactor) { visit(); }
    public void visit(BoolFactor BoolFactor) { visit(); }
    public void visit(CharFactor CharFactor) { visit(); }
    public void visit(NumFactor NumFactor) { visit(); }
    public void visit(Var Var) { visit(); }
    public void visit(FactorDesignator FactorDesignator) { visit(); }
    public void visit(NoMulopListDecls NoMulopListDecls) { visit(); }
    public void visit(MulopListDecls MulopListDecls) { visit(); }
    public void visit(NoAddopList NoAddopList) { visit(); }
    public void visit(AddopListDecl AddopListDecl) { visit(); }
    public void visit(Term Term) { visit(); }
    public void visit(NoOptionalMinusAdd NoOptionalMinusAdd) { visit(); }
    public void visit(OptionalMinusAdd OptionalMinusAdd) { visit(); }
    public void visit(Expr Expr) { visit(); }
    public void visit(Type Type) { visit(); }
    public void visit(Assignop Assignop) { visit(); }
    public void visit(LessOREqualOp LessOREqualOp) { visit(); }
    public void visit(LessOp LessOp) { visit(); }
    public void visit(GreaterOREqualOp GreaterOREqualOp) { visit(); }
    public void visit(GreaterOp GreaterOp) { visit(); }
    public void visit(NotEqualToOp NotEqualToOp) { visit(); }
    public void visit(EqualToOp EqualToOp) { visit(); }
    public void visit(SubopDecl SubopDecl) { visit(); }
    public void visit(AddopDecl AddopDecl) { visit(); }
    public void visit(ModDecl ModDecl) { visit(); }
    public void visit(DivDecl DivDecl) { visit(); }
    public void visit(MulDecl MulDecl) { visit(); }
    public void visit(NoMethodVarDeclarations NoMethodVarDeclarations) { visit(); }
    public void visit(MethodVarDeclarations MethodVarDeclarations) { visit(); }
    public void visit(NoFormParsAddList NoFormParsAddList) { visit(); }
    public void visit(FormParsAddList FormParsAddList) { visit(); }
    public void visit(FormParsType FormParsType) { visit(); }
    public void visit(FormParsDeclVar FormParsDeclVar) { visit(); }
    public void visit(FormParsDeclArray FormParsDeclArray) { visit(); }
    public void visit(DoNotHaveFormalParms DoNotHaveFormalParms) { visit(); }
    public void visit(HaveFormalParms HaveFormalParms) { visit(); }
    public void visit(MethodDeclTypeName MethodDeclTypeName) { visit(); }
    public void visit(NoReturnMethodType NoReturnMethodType) { visit(); }
    public void visit(ReturnMethodType ReturnMethodType) { visit(); }
    public void visit(MethodDecl MethodDecl) { visit(); }
    public void visit(NoVarDeclListModification NoVarDeclListModification) { visit(); }
    public void visit(VarDeclListModification VarDeclListModification) { visit(); }
    public void visit(RecordName RecordName) { visit(); }
    public void visit(RecordDecl RecordDecl) { visit(); }
    public void visit(OnlyVariableDecl OnlyVariableDecl) { visit(); }
    public void visit(VariableDecl VariableDecl) { visit(); }
    public void visit(NoVarDeclaration NoVarDeclaration) { visit(); }
    public void visit(VarDeclListDerived1 VarDeclListDerived1) { visit(); }
    public void visit(VarDeclaration VarDeclaration) { visit(); }
    public void visit(VarDecl VarDecl) { visit(); }
    public void visit(BoolConstant BoolConstant) { visit(); }
    public void visit(CharConstant CharConstant) { visit(); }
    public void visit(NumConstant NumConstant) { visit(); }
    public void visit(ConstName ConstName) { visit(); }
    public void visit(ConstDeclOne ConstDeclOne) { visit(); }
    public void visit(ConstDeclListDerived1 ConstDeclListDerived1) { visit(); }
    public void visit(NoConstListDeclarations NoConstListDeclarations) { visit(); }
    public void visit(ConstListDeclarations ConstListDeclarations) { visit(); }
    public void visit(ConstDecl ConstDecl) { visit(); }
    public void visit(GroupDeclarationRecords GroupDeclarationRecords) { visit(); }
    public void visit(GroupDeclarationVars GroupDeclarationVars) { visit(); }
    public void visit(GroupDeclarationsConsts GroupDeclarationsConsts) { visit(); }
    public void visit(NoGroupDeclarations NoGroupDeclarations) { visit(); }
    public void visit(GroupDeclarations GroupDeclarations) { visit(); }
    public void visit(ProgramName ProgramName) { visit(); }
    public void visit(NoStatement NoStatement) { visit(); }
    public void visit(MultipleMethodDecl MultipleMethodDecl) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
